"""Run a few-shot pipeline sweep configuration.

Usage: python benchmarks/run_sweep_fewshot.py <model_name> [config_label]
"""
...
from pipeline_fewshot import FewShotPipeline
...
def score(predicted, expected, mode):
    ...
    if mode == "set":
        ...
        if not missing:
            return {"correct": True, "kind": "set_superset", "notes": "extra=" + str(extra)}
        ...
        if pred_num is None:
            return {"correct": False, "kind": "no_number", "notes": ""}
        ok = (exp_num == 0 and pred_num == 0) or (exp_num != 0 and abs(pred_num - exp_num) / abs(exp_num) <= 0.01)
        return {"correct": ok, "kind": "numeric", "notes": "pred=" + str(pred_num) + " exp=" + str(exp_num)}
    else:
        pred_num = extract_first_number(pred_text)
        try:
            exp_num = float(expected)
            ...
            pass
        if pred_text.lower().strip() == str(expected).lower().strip():
            return {"correct": True, "kind": "exact_string", "notes": ""}
        if str(expected).lower().strip() in pred_text.lower().strip():
            return {"correct": True, "kind": "exact_substring", "notes": ""}
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python benchmarks/run_sweep_fewshot.py <model> [label]")
        sys.exit(1)

    model_name = sys.argv[1]
    config_label = sys.argv[2] if len(sys.argv) > 2 else ("fewshot_" + model_name)
    ...
    print("[*] Strategy: monolithic with few-shot examples")

    pipeline = FewShotPipeline(model=model_name)
    print("[*] Warming up ...")
    ...
    for i, q in enumerate(questions, start=1):
        try:
            ...
            print("  [{}/{}] {} {} {} ({:.0f}ms)".format(
                str(i).rjust(2), len(questions), verdict, q["id"], q["category"], r.latency.total_ms))
            results.append({
                "id": q["id"], "category": q["category"], "question": q["question"],
                "expected": q["expected_answer"], "expected_mode": q["expected_match_mode"],
                "predicted": r.answer, "correct": sc["correct"],
                "score_kind": sc["kind"], "score_notes": sc["notes"],
                "context_chars": r.context_chars, "prompt_chars": r.prompt_chars,
                "model": r.model, "latency": asdict(r.latency),
                "prompt_eval_count": r.prompt_eval_count, "eval_count": r.eval_count,
            })
        except Exception as e:
            print("  [{}/{}] ERROR {}: {}".format(i, len(questions), q["id"], str(e)[:120]))
            results.append({"id": q["id"], "category": q["category"], "error": str(e)[:200]})
    ...
    summary = {
        "config_label": config_label, "model": model_name,
        "backend": "ollama", "prompt_strategy": "monolithic_fewshot",
        "total_questions": len(questions), "valid_runs": len(valid),
        "correct": len(correct), "accuracy": len(correct) / len(valid) if valid else 0,
        "wall_clock_seconds": total_elapsed,
        "latency_ms": {"p50_total": p50, "p95_total": p95, "mean_total": mean},
        "by_category": {
            cat: {
